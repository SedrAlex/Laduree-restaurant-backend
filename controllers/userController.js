...
const sendLoginCode = asyncHandler(async (req, res) => {
  ...
  const reply_to = "noreply@Sedra.com";
...
//Hash token and save
 ...
 await new Token({
  ...
  vToken: hashedToken,
  createdAt: Date.now(),
  expiresAt: Date.now() + 60 * (60 * 100) //60 minutes
 }).save()
 
 // Construct Verification URL
 const verificarionUrl = `${process.env.FRONTEND_URL}/
...
//Logout User
 const logoutUser = asyncHandler( async(req, res) =>
 {
  res.cookie("token","", {
    path: "/",
    httpOnly: true,
    expires: new Date(0), //Immediatly expires
    sameSite: "none",
    secure: true,
}); 
...
const changePassword = asyncHandler(async (req, res) => {
  ...
  if (user && passwordIsCorrect) {
    ...
    res
      .status(200)
      .json({ message: "Password changed successful, please login again" });
  } else {
    res.status(400);
